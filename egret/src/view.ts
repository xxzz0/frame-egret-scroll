...
namespace view{
    ...
    /**预设的宽度 */
    export let contentWidth:number=eval(egretPlayer.dataset.contentWidth);
    /**页面间距 */
    ...
    export function init(stage:egret.Stage,main:Main){
        view.stage=stage;
        view.mainBox=main;
        margin=(stage.stageWidth-contentWidth)/2;
    }
    //视图基类
    export abstract class Base extends egret.DisplayObjectContainer{
        /**
         * @param isFixedHeight {boolean} 是否固定宽度
         */
        public constructor(isFixedWidth?:boolean) {
            super();
            if(isFixedWidth){
            ...
            this.y=this.anchorOffsetY=stage.stageHeight/2;
